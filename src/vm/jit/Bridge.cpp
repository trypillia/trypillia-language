...
extern "C" double jit_mod_helper(double a, double b) { return std::fmod(a, b); }
...
extern "C" double jit_property_get_helper(void* vm_ptr, double object_val,
  ...
  if (isObj) {
    ...
    if (obj->type == ObjType::OBJ_INSTANCE) {
      ...
      if (klass->statics.count(propName)) result = klass->statics[propName];
    }
  }
  double ret;
  memcpy(&ret, &result, sizeof(double));
  ...
  if (isObj) {
    ...
    if (obj->type == ObjType::OBJ_NATIVE) {
      ...
      if (vm->compiledFuncs.count(function)) {
        ...
        result = nativeJitFunc(vm, freshArgs.data(), argCount,
                               argCount > 0 ? freshArgs[1] : 0.0);
...
extern "C" double jit_create_closure_helper(void* vm_ptr, double func_val,
                                            double* upvalue_data,
                                            int upvalue_count) {
  VM* vm = static_cast<VM*>(vm_ptr);
  uint64_t funcRaw;
...
extern "C" double jit_get_upvalue_helper(void* vm_ptr, int slot) {
  VM* vm = static_cast<VM*>(vm_ptr);
  if (vm->jitClosure && slot < (int)vm->jitClosure->upvalues.size() &&
      vm->jitClosure->upvalues[slot]) {
