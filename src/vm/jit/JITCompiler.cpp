...
JitFunc JITCompiler::compileMathFunction(ObjFunction *function)
{
    ...
    // Collect type guard failures for deopt path
    std::vector<struct sljit_jump *> deoptJumps;
    ...
    for (size_t i = 0; i < function->chunk->code.size(); ++i)
    {
        ...
        case static_cast<uint8_t>(OpCode::OP_ADD): {
            ...
            // Type guard: both operands must be numbers
            {
                ...
                struct sljit_jump *failB = nullptr;
                emitter.emitTypeCheck(sp - 2, &failA);
                emitter.emitTypeCheck(sp - 1, &failB);
    ...
    emitter.emitDeoptPath(deoptJumps);
