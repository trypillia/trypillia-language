...
JitFunc JITCompiler::compileMathFunction(ObjFunction *function) {
  ...
  if (function->chunk->code.size() > 10 &&
      function->chunk->code[0] == static_cast<uint8_t>(OpCode::OP_GET_LOCAL) &&
      function->chunk->code[1] == 0 &&
    ...
    if (val.isNumber() &&
        function->chunk->code[4] == static_cast<uint8_t>(OpCode::OP_LESS)) {
      hasBaseCase = true;
  ...
  for (size_t i = 0; i < function->chunk->code.size(); ++i) {
    ...
    case static_cast<uint8_t>(OpCode::OP_CONSTANT): {
      ...
      uint8_t nextOp = (i + 1 < function->chunk->code.size())
                           ? function->chunk->code[i + 1]
                           : 0;
      if (val.isNumber() &&
          (nextOp == static_cast<uint8_t>(OpCode::OP_ADD) ||
    ...
    case static_cast<uint8_t>(OpCode::OP_CONSTANT_WIDE): {
      uint16_t idx = static_cast<uint16_t>((function->chunk->code[i + 1] << 8) |
                                           function->chunk->code[i + 2]);
      i += 2;
      VMValue val = function->chunk->constants[idx];
      flushTos(sp);
      double raw;
    ...
    case static_cast<uint8_t>(OpCode::OP_JUMP_IF_FALSE): {
      uint16_t offset =
          (function->chunk->code[i + 1] << 8) | function->chunk->code[i + 2];
    ...
    case static_cast<uint8_t>(OpCode::OP_CALL): {
      ...
      if (hasBaseCase && argCount == 1) {
        ...
        emitter.emitRecursiveFastPath(calleeSp + 1, baseCaseThreshold,
                                      &isBaseCase);

        // SLOW PATH: Real recursive call
        emitter.emitCallDynamic(calleeSp, calleeSp, argCount);
        struct sljit_jump *recursiveEnd =
            sljit_emit_jump(emitter.getCompiler(), SLJIT_JUMP);
