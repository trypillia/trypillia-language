...
bool Serializer::buildStandalone(ObjFunction *function,
                                 const std::string &trypilliaExePath,
  ...
  dst.write(reinterpret_cast<const char *>(&bytecodeSize),
            sizeof(bytecodeSize));
  dst.write("TRYS", 4);
...
ObjFunction *
Serializer::loadEmbeddedBytecode(const std::string &currentExePath) {
...
void Serializer::writeFunction(std::ofstream &out, ObjFunction *function) {
  ...
  out.write(reinterpret_cast<const char *>(&function->arity),
            sizeof(function->arity));
  out.write(reinterpret_cast<const char *>(&function->maxArity),
            sizeof(function->maxArity));
  out.write(reinterpret_cast<const char *>(&function->isAbstract),
            sizeof(function->isAbstract));
...
ObjFunction *Serializer::readFunction(std::ifstream &in) {
  ...
  in.read(reinterpret_cast<char *>(&function->maxArity),
          sizeof(function->maxArity));
  in.read(reinterpret_cast<char *>(&function->isAbstract),
          sizeof(function->isAbstract));
...
void Serializer::writeChunk(std::ofstream &out, Chunk *chunk) {
  ...
  if (linesSize > 0)
    out.write(reinterpret_cast<const char *>(chunk->lines.data()),
              linesSize * sizeof(int));
...
Chunk *Serializer::readChunk(std::ifstream &in) {
  ...
  if (linesSize > 0) {
    ...
    in.read(reinterpret_cast<char *>(chunk->lines.data()),
            linesSize * sizeof(int));
  }
