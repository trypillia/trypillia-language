...
public:
  ...
  void emitByte(uint8_t byte) { chunk->write(byte, currentLine); }
  ...
  void visit(BreakStmt *node) override {
    ...
    for (int i = static_cast<int>(locals.size()) - 1;
         i >= 0 && locals[i].depth > depth; i--) {
      emitByte(static_cast<uint8_t>(OpCode::OP_POP));
    }
  ...
  void visit(ForeachStmt *node) override {
    ...
    loops.push_back({loopStart, scopeDepth, {}, {}});
    emitBytes(static_cast<uint8_t>(OpCode::OP_GET_LOCAL),
              static_cast<uint8_t>(iterableLocal));
    emitBytes(static_cast<uint8_t>(OpCode::OP_GET_LOCAL),
  ...
  void visit(FieldDeclNode *node) override {
    if (!currentClassName.empty()) {
      emitBytes(static_cast<uint8_t>(OpCode::OP_GET_GLOBAL),
                static_cast<uint8_t>(
                    chunk->addConstant(resolveName(currentClassName))));
  ...
  void compileMethod(FunctionNode *node, ClassNode *classNode = nullptr) {
    ...
    if (node->isStatic) {
      ...
      emitBytes(static_cast<uint8_t>(OpCode::OP_METHOD),
                static_cast<uint8_t>(chunk->addConstant(node->name)));
    }
  }
  ...
  void visit(ClassNode *node) override {
    ...
    currentParentName =
        node->parentName.empty() ? "" : resolveName(node->parentName);
  ...
  void visit(StaticGetExpr *node) override {
    ...
    if (arg != -1) {
      ...
      emitBytes(static_cast<uint8_t>(OpCode::OP_GET_GLOBAL),
                static_cast<uint8_t>(
                    chunk->addConstant(resolveName(node->className.lexeme))));
    }
    emitBytes(
  ...
  void visit(LoadStmt *node) override {
    ...
    if (!file.is_open()) {
      ...
      std::string libPath =
          cwd + "/lib/" + path.substr(path.find_last_of('/') + 1);
      file.open(libPath);
      if (file.is_open()) {
        path = libPath;
      }
    ...
    if (!file.is_open()) {
      std::cerr << "Compile error: Could not load file '" << path << "'"
                << std::endl;
      return;
    }
  ...
  void visit(DictExpr *node) override {
    ...
    emitBytes(static_cast<uint8_t>(OpCode::OP_BUILD_MAP),
              static_cast<uint8_t>(node->elements.size()));
  }
...
ObjFunction *Compiler::compile(ASTNode *ast, SymbolTable *globals) {
  ...
  visitor.emitBytes(static_cast<uint8_t>(OpCode::OP_NIL),
                    static_cast<uint8_t>(OpCode::OP_RETURN));
