...
#else
static LONG WINAPI
windowsStackOverflowHandler(EXCEPTION_POINTERS* ExceptionInfo) {
...
InterpretResult VM::run(int targetFrameDepth) {
  ...
  for (;;) {
    ...
    if (collectCoverage) {
      ...
      if (offset < chunk->coverage.size()) chunk->coverage[offset]++;
    }
    switch (instruction) {
      ...
      case static_cast<uint8_t>(OpCode::OP_LOOP): {
        if (bytesAllocated > nextGC) GC::collect(this);
        uint16_t offset = READ_SHORT();
        frame->ip -= offset;
        break;
      }
      case static_cast<uint8_t>(OpCode::OP_ITER_HAS_NEXT): {
        ...
        return runtimeError(
            std::string("Invalid operand types for iteration."));
      }
      ...
      case static_cast<uint8_t>(OpCode::OP_PROPERTY_SET): {
        ...
        if (instanceVal.isInstance()) {
          ...
          if (!checkAccess(mod, instance->klass, callerClass)) {
            return runtimeError(std::string("Access error: Cannot set '") +
                                name + "'.");
          }
