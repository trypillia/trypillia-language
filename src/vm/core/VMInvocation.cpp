...
bool VM::executePropertyGet(const std::string& name) {
  ...
  std::string callerClass =
      frames.back().closure
          ? frames.back().closure->function->enclosingClassName
          : "";
  ...
  if (instanceVal.isInstance()) {
    ...
    if (instance->fields.count(name)) {
      ...
      if (!checkAccess(mod, instance->klass, callerClass)) {
        runtimeError(std::string("Access error: Cannot access method '") +
                     name + "'.");
        return false;
      }
      pop();  // instance
    ...
    if (klass->statics.count(name)) {
      ...
      if (methodVal.isClosure()) {
        ...
        if (!checkAccess(func->accessModifier, klass, callerClass)) {
          runtimeError(
              std::string("Access error: Cannot access static method '") +
              name + "'.");
          return false;
        }
      }
...
bool VM::executeCall(uint8_t argCount) {
  ...
  if (callee.isClosure()) {
    ...
    if (function->arity != -1 &&
        (argCount < function->arity || argCount > function->maxArity)) {
      std::string expected = function->arity == function->maxArity
                                 ? std::to_string(function->arity)
                                 : std::to_string(function->arity) + "-" +
                                       std::to_string(function->maxArity);
      runtimeError(std::string("Expected ") + expected + " arguments but got " +
    ...
    if (compiledFuncs.count(funcPtr)) {
      ...
      if (nativeJitFunc) {
        ...
        std::cerr << "JIT ABORTED for " << function->name << " at call #"
                  << funcPtr->callCount << std::endl;
      }
    } else {
      funcPtr->callCount++;
    ...
    if (klass->isAbstract) {
      runtimeError(std::string("Cannot instantiate abstract class '") +
                   klass->name + "'.");
      return false;
    }
    ...
    if (klass->methods.count("init")) {
      ...
      if (initMethod.isClosure()) {
        ...
        newFrame.stackStart =
            static_cast<int>((stackTop - stack) - argCount - 1);
        frames.push_back(newFrame);
    ...
    if (isMethodAbstract(function)) {
      runtimeError(std::string("Cannot call abstract method '") +
