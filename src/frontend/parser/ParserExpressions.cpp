...
ExprNode *Parser::expression() { return parsePrecedence(PREC_ASSIGNMENT); }

ExprNode *Parser::parsePrecedence(Precedence precedence) {
  advance();
  ParsePrefixFn prefixRule = getRule(previousToken.type).prefix;
  if (prefixRule == nullptr) {
    throw std::runtime_error("Expect expression at '" + previousToken.lexeme +
                             "' line " + std::to_string(previousToken.line));
  }
...
ExprNode *Parser::parseAssignment(ExprNode *left, bool canAssign) {
  ...
  if (op.type == TokenType::ASSIGN) {
    if (VariableExpr *varExpr = dynamic_cast<VariableExpr *>(left)) {
      ...
      return new StaticSetExpr(statExpr->className, statExpr->memberName,
                               value);
    }
    throw std::runtime_error("Invalid assignment target");
  } else {
