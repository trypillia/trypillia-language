...
int main(int argc, char **argv) {
  ...
  if (argIdx >= argc) {
    std::cerr << "Usage: " << argv[0] << " [--coverage] [build] <file> [output]"
              << std::endl;
    return 1;
  }
  ...
  if (command == "build") {
    argIdx++;
    if (argIdx >= argc) {
      std::cerr << "Usage: " << argv[0] << " build <file.try> [output]"
                << std::endl;
      return 1;
    }
  ...
  if (!sourceFile.is_open()) {
    std::cerr << "Error: Could not open source file: " << inputFile
              << std::endl;
    return 1;
  }
  ...
  if (function) {
    if (buildStandalone) {
      if (Serializer::buildStandalone(function, exePath, outputFile)) {
        std::cout << "Successfully built standalone executable: " << outputFile
                  << std::endl;
      } else {
        std::cerr << "Error building standalone executable: " << outputFile
                  << std::endl;
        return 1;
      }
    } else {
