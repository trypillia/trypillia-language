...
static VMValue resolveNative(int argCount, VMValue* args) {
  ...
  for (size_t i = 0; i < handlers.size(); i += 3) {
    ...
    pm.targetPromise =
        handlers[i + 2].isPromise() ? handlers[i + 2].asPromise() : nullptr;
...
static VMValue thenNative(int argCount, VMValue* args) {
  ...
  if (promise->resolved) {
    VM::PromiseMicrotask pm;
    pm.onFulfilled =
        argCount > off && args[off].isClosure() ? args[off] : VMValue(nullptr);
    pm.onRejected = argCount > off + 1 && args[off + 1].isClosure()
                        ? args[off + 1]
                        : VMValue(nullptr);
  ...
  promise->thenHandlers.push_back(
      argCount > off && args[off].isClosure() ? args[off] : VMValue(nullptr));
  promise->thenHandlers.push_back(
      argCount > off + 1 && args[off + 1].isClosure() ? args[off + 1]
                                                      : VMValue(nullptr));
