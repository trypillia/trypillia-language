...
static void workerThreadEntry(std::string scriptPath,
                              std::shared_ptr<WorkerChannel> channel) {
...
static VMValue workerSelfReceive(int argCount, VMValue* args) {
  ...
  currentWorkerChannel->mtwCond.wait(lock, [] {
    return !currentWorkerChannel->mainToWorker.empty() ||
           !currentWorkerChannel->isAlive;
  });
...
void registerAll(VM* vm) {
  ...
  workerClass->statics["selfSend"] =
      new ObjNative("selfSend", 1, workerSelfSend);
  workerClass->statics["selfReceive"] =
      new ObjNative("selfReceive", 0, workerSelfReceive);
