...
static VMValue assertEqNative(int argCount, VMValue *args) {
  if (argCount < 2) {
    failAssertion(
        currentVM,
        "FAIL: assertEq requires at least 2 arguments (actual, expected)");
    return nullptr;
  }
...
static VMValue assertNeqNative(int argCount, VMValue *args) {
  if (argCount < 2) {
    failAssertion(
        currentVM,
        "FAIL: assertNeq requires at least 2 arguments (actual, unexpected)");
    return nullptr;
  }
...
static VMValue assertThrowsNative(int argCount, VMValue *args) {
  ...
  if (setjmp(vm->catchJmpBuf) == 0) {
    ...
    if (result == InterpretResult::INTERPRET_OK) {
      ...
      failAssertion(vm, "FAIL: " + loc +
                            "expected function to throw but it did not" + msg);
    }
  } else {
    vm->catchJumpEnabled = false;
    vm->suppressRuntimeErrors = false;
...
static VMValue fitNative(int argCount, VMValue *args) {
  ...
  if (!name.isString()) {
    failAssertion(vm,
                  "FAIL: fit() first argument must be a string (test name)");
    return nullptr;
  }
  if (!fn.isClosure()) {
