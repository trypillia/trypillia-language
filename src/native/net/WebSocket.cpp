...
static VMValue wsServerCreate(int argCount, VMValue *args) {
  ...
  if (mbedtls_net_bind(&data->fd, NULL, std::to_string(port).c_str(),
...
static std::string extractHeader(const std::string &request,
...
static VMValue wsServerAccept(int argCount, VMValue *args) {
  ...
  std::string acceptKey =
      res.asInstance()->fields["value"].asString()->flatten();
  std::string response =
      "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: "
      "Upgrade\r\nSec-WebSocket-Accept: " +
      acceptKey + "\r\n\r\n";
  mbedtls_net_send(&client_fd, (const unsigned char *)response.c_str(),
                   response.length());
...
static VMValue wsSend(int argCount, VMValue *args) {
  ...
  return makeResultOk(
      currentVM, mbedtls_net_send(&data->fd, frame.data(), frame.size()) >= 0);
}
...
static VMValue wsRecv(int argCount, VMValue *args) {
  ...
  size_t total = 0;
  while (total < len) {
    int r = mbedtls_net_recv(&data->fd, payload.data() + total,
